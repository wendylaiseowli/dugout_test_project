...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (flagContainer && dropdown && arrow) {
    // 🔹 Toggle on flag click
    ...
    // 🔹 Close dropdown when selecting a country
    ...
    .addEventListener("keyup", function (event) {
      ...
      if (isValidNumber.valid) {
        ...
        phoneLabel.innerText =
          "International Telephone Input is Valid (" +
          isValidNumber.valid +
          ")";
      } else {
        ...
        phoneLabel.innerText =
          "International Telephone Input is Not Valid (" +
          isValidNumber.valid +
          ")";
      }
    });
...
var intlTelInput = {
  ...
  phoneNumberValidator: {
    ...
    JP: [
      ,
      [, , "[1-9]\\d{8,9}|00(?:[36]\\d{7,14}|7\\d{5,7}|8\\d{7})", "\\d{8,17}"],
      [
        ,
        ,
    ...
    RS: [
      ,
      [, , "[126-9]\\d{4,11}|3(?:[0-79]\\d{3,10}|8[2-9]\\d{2,9})", "\\d{5,12}"],
      [
        ,
        ,
  ...
  if (element.prefixes) {
    if (element.prefixes.length) {
      for (var j = 0; j < element.prefixes.length; j++) {
        countryCodes[element.dialCode + element.prefixes[j]] = [
          element.cca2.toUpperCase(),
        ];
      }
    } else {
      countryCodes[element.dialCode] =
        intlTelInput.countryCodes[element.dialCode];
    }
  } else {
    countryCodes[element.dialCode] =
      intlTelInput.countryCodes[element.dialCode];
...
function intlTelInputInit(elements) {
  ...
  if (
    typeof intlTelInput.preferredCountries !== "undefined" ||
    intlTelInput.preferredCountries === null
  ) {
  ...
  elements.forEach(function (element) {
    ...
    if (intlTelInput.countries.length)
      _appendListItems(countryList, intlTelInput.countries, "standard");
    ...
    element.addEventListener("keyup", function (e) {
      ...
      if (dialCodeArray.prefixesDialCode) {
        var selectedFlagInner =
          countryList.previousElementSibling.querySelector(
            ".selected-flag .flag"
          );
        var countryCode = _getCountryCode(
          element.value,
          dialCodeArray.prefixesDialCode
        );

        // check if one of the matching country's is already selected
        if (!selectedFlagInner.classList.contains(countryCode))
          _selectFlag(countryList, countryCode);
      }
    });
  });
  ...
  document.addEventListener("keydown", function (e) {
    ...
    if (e.key === "ArrowUp")
      _scrollTo(countryListItems[countryListItemIndex - 1]); // up

    if (e.key === "ArrowDown")
      _scrollTo(countryListItems[countryListItemIndex + 1]); // down

    if (e.key === "Enter")
      _selectListItem(
        countryListItems[countryListItemIndex],
  ...
  function _selectFlag(countryList, countryCode) {
    ...
    var selectedFlagInner = countryList.previousElementSibling.querySelector(
      ".selected-flag .flag"
    );
    selectedFlagInner.setAttribute(
      "class",
  ...
  function _selectListItem(countryListItem, countryListItemIndex) {
    ...
    selectedFlagInner.setAttribute(
      "class",
      `flag ${selectedCountries[countryListItemIndex].cca2.toLowerCase()}`
    );

    // update input value
    var telInputElement = countryList.nextElementSibling;
    var newNumber = _updateNumber(
      telInputElement.value,
      selectedCountries[countryListItemIndex].dialCode
    );
  ...
  function _scrollTo(countryListItem, itemScrolled = true) {
    if (
      ...
      typeof countryListItem === "undefined"
    )
      return;

    var countryList = countryListItem.parentElement;
    var countryListItems = countryList.querySelectorAll("li.country");
    var countryListItemIndx = [].indexOf.call(
      countryListItems,
      countryListItem
    );
    var countryListItemIndex =
      Array.from(countryListItems).indexOf(countryListItem);
    ...
    if (itemScrolled) {
      if (
        itemOffset + itemHeight < scrollTop ||
        itemOffset + itemHeight > scrollTop + viewport
      )
  ...
  function _closeDropdown() {
    elements.forEach(function (element) {
      ...
      var arrow =
        element.previousElementSibling.previousElementSibling.querySelector(
          ".arrow"
        );
      arrow.classList.remove("arrow-up");
    });
  ...
  function _countryListEventHandler(countryList) {
    ...
    var countryListItems = countryList.querySelectorAll("li.country");
    var selectedFlag =
      countryList.previousElementSibling.querySelector(".selected-flag");

    // Selected Flag Click Event
    // Selected Flag Click Event (robust toggle + avoid duplicate handlers)
    if (!selectedFlag.dataset._flagClickBound) {
      selectedFlag.addEventListener("click", function (e) {
        ...
        // Otherwise, close any other dropdowns and open this one
        _closeDropdown();
        ...
        if (activeCountryListItem) {
          ...
          // same scroll / wheel handling as before:
          ...
          function disableScroll(element) {
            element.addEventListener("scroll", preventDefault, false);
          ...
          function enableScroll(element) {
            element.removeEventListener("scroll", preventDefault, false);
          ...
          countryList.addEventListener(
            ...
            function (ev) {
              ...
              } else if (Math.sign(ev.deltaY) > 0) {
                countryList.scrollHeight - countryList.offsetHeight <=
      ...
      // mark as bound so we don't attach another identical handler when re-appending items
      selectedFlag.dataset._flagClickBound = "1";
    }
...
function isValidNumber(inputVal, dialCode, countryCode) {
  ...
  if (
    ...
    valid =
      -1 !=
      dialCodePrefixesValidator(
        splitPhoneNumber.prefixes,
        intlTelInput.phoneNumberValidator[countryCode]
      );
  }
  return { valid: valid };
}
...
function _getDialCode(inputVal) {
  ...
  if (inputVal.charAt(0) == "+") {
    ...
    for (var i = 0; i < inputVal.length; i++) {
      if (
        ...
        countryCodes[prefixesDialCode] &&
          (PrefixesCountryDialCode = inputVal.substr(0, i + 1)),
        dialCode.length == 4)
      )
        break;
  ...
  return {
    dialCode: countryDialCode.replace(/\D/g, ""),
    prefixesDialCode: PrefixesCountryDialCode.replace(/\D/g, ""),
  };
}
  ...
  if (countryCodeArray == null) {
    ...
    for (var i = 0; i < countryCodeArray.length; i++) {
      if (intlTelInput.phoneNumberValidator[countryCodeArray[i]][23] != null) {
        if (
          0 ==
          splitPhoneNumber.prefixes.search(
            intlTelInput.phoneNumberValidator[countryCodeArray[i]][23]
          )
        ) {
        ...
        dialCodePrefixesValidator(
          splitPhoneNumber.prefixes,
          intlTelInput.phoneNumberValidator[countryCodeArray[i]]
        )
      ) {
        countryCode = countryCodeArray[i];
        break;
...
function PrefixesValidator(prefixes, validator) {
  return (
    PrefixesValidatorMatch(prefixes, validator[3]) &&
    PrefixesValidatorMatch(prefixes, validator[2])
  );
}
