...
<html lang="en">
  <head>
    ...
    <title>Dashboard | The Dugout Oasis Admin Panel</title>
    <!-- CSS -->
    <link
  ...
  <body class="sidebar-dark sidebar-expand navbar-brand-dark header-light">
    ...
    <input type="hidden" name="_token" value="tGnjwKvz0npHCb76IqzaP6r3g47HISn6rrwSEBj4">
    ...
    <div id="wrapper" class="wrapper">
      ...
      <nav class="navbar">
        ...
        <div class="navbar-header">
          <a href="{{route('dashboard')}}" class="navbar-brand">
            <img class="logo-expand ds-logo" alt="" src="{{ asset('img/admin/dugout-logo-white_new.png') }}">
            <img class="logo-collapse ds-logo" alt="" src="{{ asset('img/admin/dugout-logo-white_new.png') }}">
            <!-- <p>BonVue</p> -->
          </a>
        </div>
        <!-- /.navbar-header -->
          ...
          <li class="dropdown">
            <a href="javascript:void(0);" class="dropdown-toggle dropdown-toggle-user ripple" data-toggle="dropdown">
              <span class="avatar thumb-xs2">
                <img src="{{ asset('img/admin/ds-img/userasset-4.png')}}" class="rounded-circle" alt="">
            ...
            <div class="col-sm-12 text-center p-0 clearfix">
              <div class="d-inline-block pos-relative mr-b-10">
                <figure class="thumb-sm mr-b-0 user--online">
                  <img src="http://xcitemedia.tv/admin-wiseowl/assets/demo/users/user1.jpg" class="rounded-circle" alt="">
                </figure>
              ...
              <div class="lh-14 mr-t-5">
                <a href="#" class="hide-menu mt-3 mb-0 side-user-heading fw-500">Scott Adams</a>
                <br>
                ...
                <ul class="nav in side-menu">
                  <li class="current-page menu-item-has-children active">
                    <a href="{{route('dashboard')}}" id="dashboard"
                      ...
                      <span class="hide-menu dugout-accent-color">Dashboard</span></a
                    >
                  </li>
        ...
        <main class="main-wrapper clearfix">
          ...
          <div class="row page-title clearfix">
            <div class="page-title-left">
              <h6 class="page-title-heading mr-0 mr-r-5">The Dugout Oasis Admin Panel</h6>
          ...
          <div class="row">
            <div class="widget-list row">
              ...
              <!-- Today's Reservation Widget -->
              <div class="widget-holder widget-sm widget-full-height">
                <div class="widget-bg">
                  <div class="widget-body">
                    <div class="counter-w-info media mb-3">
                      <!-- <div class="counter-w-info media mb-3"
                      onclick="window.location.href = document.getElementById('baseURL').value + '/contact-form-list';"> -->
                      <div class="media-body">
                        <p class="mr-b-5 text-black">Today's Reservation</p>
                        <span class="counter-title"><span class="counter">{{$todayReservationCount}}</span></span>
                      </div>
                      <!-- /.media-body -->
                      <div class="pull-right align-self-center">
                        <i class="list-icon fa fa-users bg-primary"></i>
                      </div>
                    </div>
                    <div class="btn btn-info dashboard-details-btn">
                      <a href="{{route('reservations.index')}}">View Details</a>
                    </div>
                  </div>
                  <!-- /.widget-body -->
                </div>
          ...
          <div class="widget-list">
            <div class="row">
              <div class="col-md-12 widget-holder">
                ...
                <!-- TODAY'S RESERVATION TABLE -->
                <div class="widget-bg">
                  <div class="widget-heading clearfix">
                    <h5 class="text-black">Today's Reservations</h5>
                  </div>
                  <div class="widget-body clearfix">
                    <div class="table-responsive">
                      <table class="table table-editable" id="indexTable">
                        <thead>
                          <tr>
                          ...
                          @if($todayReservationCount>0)
                            @foreach ($todayReservations as $todayReservation)
                              <tr>
                                ...
                                <td>{{ $todayReservation->reservation_name }}</td>
                                <td>{{ $todayReservation->reservation_date->format('Y-m-d') }}</td>
                                <td>{{ $todayReservation->reservation_time->format('h:i A') }}</td>
                                <td>{{ $todayReservation->number_of_people }}</td>
                                <td>{{ $todayReservation->phone_number }}</td>
                                <td>{{ $todayReservation->email }}</td>
                              </tr>
                            @endforeach
                          @else
                            <tr>
                              <td colspan="7" class=" text-center py-4">
                                There is no reservation for today
                              </td>
                            </tr>
                          @endif
                        </tbody>
                ...
                <!-- LAST 10 PUNDIT WINNERS TABLE -->
                <div class="widget-bg">
                  <div class="widget-heading clearfix">
                    <h5 class="text-black">Last 10 Pundit Winners</h5>
                  </div>
                  <div class="widget-body clearfix">
                    <div class="table-responsive">
                      <table class="table table-editable" id="indexTable">
                        <thead>
                          <tr>
                            ...
                            <th class="text-black">Email</th>
                            <th class="text-black">Home Team Score</th>
                            <th class="text-black">Away Team Score</th>
                          </tr>
                        </thead>
                        <tbody>
                          @if($lastTenWinners->count()>0)
                            @foreach($lastTenWinners as $lastTenWinner)
                              <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $lastTenWinner->name }}</td>
                                <td>{{ $lastTenWinner->email }}</td>
                                <td>{{ $lastTenWinner->homeTeamResult }}</td>
                                <td>{{ $lastTenWinner->awayTeamResult }}</td>
                              </tr>
                            @endforeach
                          @else
                            <tr>
                              <td colspan="7" class=" text-center py-4">
                                There is no winners record yet
                              </td>
                            </tr>
                          @endif
                        </tbody>
                ...
                <!-- LAST 10 RESERVATIONS TABLE -->
                <div class="widget-bg">
                  <div class="widget-heading clearfix">
                    <h5 class="text-black">Last 10 Reservation</h5>
                  </div>
                  <div class="widget-body clearfix">
                    <div class="table-responsive">
                      <table class="table table-editable" id="indexTable">
                        ...
                        <tbody>
                          @if($lastTenReservations->count()>0)
                            @foreach($lastTenReservations as $lastTenReservation)
                              <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $lastTenReservation->reservation_name }}</td>
                                <td>{{ $lastTenReservation->reservation_date->format('Y-m-d') }}</td>
                                <td>{{ $lastTenReservation->reservation_time->format('h:i A') }}</td>
                                <td>{{ $lastTenReservation->number_of_people }}</td>
                                <td>{{ $lastTenReservation->phone_number }}</td>
                                <td>{{ $lastTenReservation->email }}</td>
                              </tr>
                            @endforeach
                          @else
                            <tr>
                              <td colspan="7" class=" text-center py-4">
                                There is no reservation record yet
                              </td>
                            </tr>
                          @endif
                        </tbody>
            ...
            <div class="modal-dialog">
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <h2 class="text-center" id="question">Are you sure you want to change the channel status?</h2>
                  <br />
                </div>
          ...
          <!-- EDIT MODAL -->
          <div id="editModal" class="modal modal-primary fade bs-modal-lg-primary" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel2" aria-hidden="true" style="display: none">
            <div class="modal-dialog modal-lg">
              <div class="modal-content">
                <div class="modal-header text-inverse">
                  <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                  <h5 class="modal-title" id="myLargeModalLabel2">Edit Channel</h5>
                </div>
                <form class="form-horizontal" method="POST" enctype="multipart/form-data" id="edit_form">
                  ...
                  <div class="modal-body">
                    <div class="row">
                      <div class="col-md-6">
                        <div class="form-group">
                          ...
                          <input type="text" class="form-control" id="editChannelName" name="name" maxlength="200" autofocus>
                    ...
                    <div class="col-md-6">
                      <div class="form-group">
                        <label class="control-label col-sm-3" for="title">Company Name:</label>
                        <div class="col-sm-9">
                          <input type="text" class="form-control" id="editCompanyName" name="company_name" maxlength="200" autofocus>
                          <p class="erroreditCompanyName text-center alert alert-danger hidden"></p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="row">
                      <div class="col-lg-12 text-center">
                        <div class="error alert alert-icon alert-danger border-danger alert-dismissible fade" role="alert">
      ...
      <footer class="footer">
        <span class="heading-font-family">
          Copyright @ 2018. All rights reserved DS-Admin by Digital Symphony
        </span>
      </footer>
    </div>
    <!-- /#wrapper -->
    ...
    <div id="updatePassword" class="modal modal-primary fade bs-modal-lg-primary" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel2" aria-hidden="true" style="display: none">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header text-inverse">
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
          ...
          <form id="updatePasswordForm" method="post">
            ...
            <div class="modal-body">
              <div class="row">
                <div class="col-md-12">
                  <div class="form-group">
                    ...
                    <input class="form-control updatePasswordInput" id="currentPassword" name="currentPassword" placeholder="Current Password" type="password" maxlength="30">
                  </div>
                </div>
                <div class="col-md-12">
                  <div class="form-group">
                    <label>New Password</label> &nbsp;
                    <input class="form-control updatePasswordInput" id="newPassword" name="newPassword" placeholder="New Password" type="password" maxlength="30">
                  </div>
                </div>
                <div class="col-md-12">
                  <div class="form-group">
                    <label>Confirm New Password</label> &nbsp;
                    <input class="form-control updatePasswordInput" id="newPassword_confirmation" name="newPassword_confirmation" placeholder="Confirm New Password" type="password" maxlength="30">
                  </div>
                </div>
              </div>
              ...
              <div class="row">
                <div class="col-lg-12 text-center">
                  <div class="error alert alert-icon alert-danger border-danger alert-dismissible fade" role="alert">
                    <i class="material-icons list-icon">not_interested</i> <strong class="error-bold"></strong> <span class="error-text"></span>
                  </div>
                </div>
              </div>
            </div>
          </form>
          <div class="modal-footer"><a href="#" id="updatePasswordSubmit" class="btn btn-info btn-rounded ripple text-left">Update</a>
            <button type="button" class="btn btn-danger btn-rounded ripple text-left" data-dismiss="modal">Cancel</button>
          </div>
        </div>
        <!-- /.modal-content -->
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            if ((data.errors)) {
              ...
              toastr.success('Successfully updated Channel details!', 'Success Alert', {
                timeOut: 5000
              });
              location.reload();
    ...
    <script type="text/javascript">
      $(document).ready(function() {
        ...
        var myTable = $('#table-two').DataTable({
          ...
          serverSide: true,
          ajax: {
            url: $('#baseURL').val() + '/datatable-sample?startDate=' + $("#startDate-sample").val() + '&endDate=' + $("#endDate-sample").val(),
            method: 'POST'
          },
          columns: [{
              data: 'user_id',
        ...
        $("#endDate-sample").on('change', function() {
          if ($(this).val() != '') {
            ...
            myTable.ajax.url('datatable-sample?startDate=' + start + '&endDate=' + end);
            myTable.ajax.reload();
          }
        });
        ...
        function cb(start, end) {
          $('#reportrange-sample span').html(start.format('DD/MM/YY') + ' - ' + end.format('DD/MM/YY'));
          $('#startDate-sample').val(start.format('MM/DD/YY')).trigger('change');
      ...
      function reactivatePopUp(id) {
        ...
        $('#delete-text').html('Are you sure to <u>reactivate</u> user <b>' + $('#name' + id).html() + '</b>?');
      }
      ...
      function deactivatePopUp(id) {
        ...
        $('#delete-text').html('Are you sure to <u>deactivate</u> user <b>' + $('#name' + id).html() + '</b>?');
      }
    </script>
